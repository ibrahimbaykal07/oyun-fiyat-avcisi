...
# --- DOLAR KURU (CANLI TCMB) ---
@st.cache_data(ttl=3600) # 1 Saat Önbellek
def get_live_dollar_rate():
    """TCMB'den canlı kur çeker, çekemezse yedek kullanır"""
    fallback_rate = 38.00 # Eğer TCMB çökerse bu kullanılır (ASLA 1.0 OLMAZ)
    try:
...
def fetch_price_smart(game_name):
    ...
    for query in list(set(search_candidates)):
        ...
        try:
            ...
            r = requests.get(url, timeout=2) # Timeout 2 saniye
            if r.status_code == 200:
                data = r.json()
                if data:
                    d = data[0]
...
# ================= ARAYÜZ BAŞLIYOR =================
scroll_to_top()
dolar_kuru = get_live_dollar_rate() # TCMB CANLI KUR
